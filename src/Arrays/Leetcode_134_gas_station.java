...
class GasStation {
    ...
    public static void findcirculartravel(int[] gas, int[] cost){
        ...
        while(startingStation < gas.length){
            ...
            if(gas[startingStation] < cost[startingStation]){
                ...
                int i = startingStation;
                while(!arrived){
                    System.out.println("i:"+i);
                    tank = tank+gas[i];
                    if(cost[i] <= tank){
                        tank = tank - cost[i];
                        i = i < (gas.length-1)? i+1 : 0;

                        if(i == startingStation && cost[i] <= tank+gas[i]){
                            arrived= true;
                            break;
                        }
                    }
